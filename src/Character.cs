...
public partial class Character : Actor, IDamagable {
	...
	public void debuffCooldowns() {
		...
		if (matchOn && Global.level.mainPlayer.readyTextOver) {
			if (invulnTime > 0) {
				if (maxInvulnTime <= 0) {
		...
		if (burnTime > 0) {
			...
			if (burnHurtCooldown >= 60) {
				...
				burnDamager?.applyDamage(
					this, false, burnWeapon, this, (int)ProjIds.Burn, overrideDamage: 1f
				);
			}
			if (burnTime <= 0) {
				removeBurn();
	...
	public override void update() {
		...
		if (pos.y > Global.level.killY && !isWarpIn() && !charState.immortal &&
			charState is not WarpOut and not Die and not BottomlessPitState
		 ) {
			if (invulnTime <= 0) {
				playSound("hurt");
		...
		isUsingEtank = usedEtank != null;


		//Wtank ammo heal
		if (usedWtank != null && hasFullAmmo()) {
			usedWtank = null;
		}
		isUsingWtank = usedWtank != null;
	...
	public virtual bool isNonDamageStatusImmune() {
		return charState.statusEffectImmune || isStatusImmune();
	}
	...
	public bool drawETankHealing() {
		...
		} else if (isUsingEtank) {
			drawETankHealingInner();
			return true;
		}	
	...
	public void drawETankHealingInner() {
	...
	public void drawWTankHealingInner() {
		Point topLeft = new Point(pos.x - 8, pos.y - 15 + currentLabelY);
		Point topLeftBar = new Point(pos.x - 7, topLeft.y + 2);
		Point botRightBar = new Point(pos.x + 7, topLeft.y + 14);
	...
	public virtual bool hasFullAmmo() {
		if (weapons.Count <= 0) return false;
		else {
			for (int i = 0; i < weapons.Count; i++) {
				if (weapons[i].ammo < weapons[i].maxAmmo) {
					return false;
				}
			}
			return true;
	...
	public override void updateCustomActorNetData(byte[] data) {
		...
		charState.stunImmune = boolData[2];
		charState.immortal = boolData[3];
		charState.invulnerable = boolData[4];

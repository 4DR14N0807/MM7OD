...
public class BassBuster : Weapon {
    ...
    public BassBuster() : base() {
        index = (int)RockWeaponIds.BassBuster;
		weaponSlotIndex = 0;
        fireRateFrames = 6;
		isStream = true;
    }
...
public class BassBusterProj : Projectile {
    public BassBusterProj(Point pos, float byteAngle, Player player, ushort? netId, bool rpc = false) :
        base(BassBuster.netWeapon, pos, 1, 0, 0.25f, player, "rock_buster_proj", 0, 0, netId, player.ownedByLocalPlayer) {
